    ...
    <main>
        <div class="bg-white rounded-md shadow-xl p-5">
            ...
            <form action="{{ route('kriteria.update', ['id' => $kriteria->id]) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-5 mb-5">
                    ...
                    {{-- <div>
                        ...
                        <x-text-input type="number" placeholder="Masukkan bobot kriteria..." class="w-full"
                            name="bobot" step="0.1" value="{{ $kriteria->bobot }}" />
                        <x-input-error :messages="$errors->get('bobot')" />
                    </div> --}}
                    <div>
                        <x-input-label>Jenis Kriteria</x-input-label>
                        <div x-data="{ selected: '{{ $kriteria->jenis }}' }" class="flex">
                ...
                <x-primary-button class="bg-green-500"><i class="fa-solid fa-rotate mr-3"></i>Update</x-primary-button>
            </form>
        </div>
    </main>
</x-app-layout>
